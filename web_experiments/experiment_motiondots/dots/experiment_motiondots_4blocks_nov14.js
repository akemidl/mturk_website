...
var event_array=[];
  ...
  for (i=0;i<num_trials_eachblock;i++){
      var event_array=event_array.concat('probe4trial-'+i.toString())
        if(i % num_trials_eachblock ==(num_trials_eachblock-1)){ //this is saying that if the remainder of i/num_trials_eachblock=(num_trials_eachblock-1). This is coz i starts from 0...
      		var event_array = event_array.concat('break')
      	}
  }
...
function Positions_outcomebar(){
    ...
    var moving_dots_timeout=500; //was using 2000, now 500
    ...
    function RunInstruct(event){
        ...
        //// Welcome Screen
        	if(currentnumber==1){
        		document.getElementById('instruct').innerHTML='<p>Welcome to the experiment!</p><p>On each trial in this task you will be asked about the average colour of moving coloured dots. The moving coloured dots will be shown for 2.5 seconds on each trial, before being covered by a grey circle. The colours of the dots and their speed will vary independently for each trial.</p><p>Press continue</p>'
...
function RunSummaryInstruct(event){
  ...
  if (eventt.indexOf('beforetrainingtrial_instruct')==0){
      ...
      document.getElementById('instruct').innerHTML='<p>In the trials in this block you will be asked to choose whether the average dot colour was more blue (blue block) or more red (right block). The boundary between "more blue" and "more red" is arbitary but constant throughout the block. Enter your choice using the left or right arrow key. A black box will appear around the correct colour so you can learn to improve. Your bonus payment will depend on your performance.</p>'
                      ...
                      //Need to then do diff things depending on trial type...
                        if (eventt.indexOf('trainingtrial')==0){
                          ...
                          if (eventt.indexOf('0')==0)
                            {document.getElementById('participantOffer').innerHTML="Move slider using the mouse to the correct 'bllue-red' colour and double click to save choice";
                            $('#participantOffer').css('color','black');
                            $('#participantOffer').show();}
                          else
                          { document.getElementById('participantOffer').innerHTML="Move slider using the mouse to the correct 'bllue-red' colour and double click to save choice";
