...
var outcomewait = 0 // wait for outcome 4 in mikes
var outcomelim = 0 // time to view the outcome
var ITI = 2000
...
for (i = 0; i <num_trials; i++) {
	...
	var eventarray = eventarray.concat('trial-part2-'+i.toString())
	/*if(i % 5 ==4){
  ...
  $("#slider").slider({
    ...
    change: function( event, ui ) {
        ...
        console.log('recognizing slider value')
				if (score>0){
					rightleft='The right urn'
				}else{
					rightleft='The left urn'
				}
				score = Math.abs(score)
				$("#slider_value").html(rightleft+' is '+String(score)+'x more likely.')
				if(score==0){
					$("#slider_value").html('Neither urn is more likely.')
				}
...
$(document).ready( function() {
	...
	*/
	$(document).bind('keyup',PressedEnter);
...
function Proceed(e){
	...
	if (canProceed==true){
		...
		console.log(eventt.indexOf('part2'))
		...
		if (eventt.indexOf('instruct')==0){
			...
			setTimeout(function() {
				...
				if(eventt.indexOf('part2')==6){
					console.log('Running Part2')
					RunTrial_Part2(eventt)
				}else{
...
function RunTrial_Part2(event) {
	...
	$('#trial_instructs').html('Now imagine that an X was drawn from the left. How ..');
...
function PressedEnter(e){
	if (canRespond==true && e.keyCode==13 ) {
		Outcome('enter')
		last_pressed='enter'
	}

	/// example trial presses left
	if(allow_instruct_to_press && e.keyCode==13){
		if (e.keyCode==13){
			last_pressed='enter'
			Proceed('enter')
		}
