...
def exp_page_effort(request):
    ...
    if request.session['MID'] is not None:
    	...
    	if Participant.objects.filter(MID=request.session['MID']).exists():
    		...
    		if('experiment effort start' not in ptime):
    			ptime = ptime+';experiment effort start:'+str(datetime.datetime.now())
    ...
    return(render_to_response('experiment_effort.html',variables))
