...
var num_dots=650; //number of dots on screen at any time...
...
var num_block1and2trials=160; //160 category boundary block trials...
...
var event_array=[];
  ...
  for (i=0;i<num_block1and2trials;i++){
      var event_array=event_array.concat('block1trial-'+i.toString())
        if(i % num_block1and2trials ==(num_block1and2trials-1)){ //this is saying that if the remainder of i/num_trials_eachblock=(num_trials_eachblock-1). This is coz i starts from 0...
      		var event_array = event_array.concat('break')
      	}
  }
...
function MakeColourEachDot(){
  ...
  if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
    {for (i = 0; i <num_block1and2trials; i++) {
      var colour_tmp=(0.5+rb_array_block1and2[i]*0.5*colour_difficulty/1000); //transforming the input from rb_array
...
function Positions_outcomebar(){
  if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0)      {num_trials_thisblock=num_block1and2trials;}  else {num_trials_thisblock=num_trials_eachblock;};    for (i=0;i<num_trials_thisblock;i++){
  ...
  function RunAfterPracticeInstruct(event){
    ...
    if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
      {trials_counter= num_block1and2trials}
...
//Function to save the slidebar position when the subject pressed enter
  ...
  var outcomeslider=function(response){
      ...
      T4 =setTimeout(function() {
        ...
        tnum=currenttrial_t
        if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
          {element.innerHTML='Completed: '+tnum+'/'+num_block1and2trials}
...
/////////////////////////////////////////////////////////////////////////////////////////////
          ...
          if (canRespondbuttons==true && e.keyCode==39) { //39 is the keycode for right arrow press, need to put in instructions...
              ...
              document.getElementById("right_choice").style.height="65%";document.getElementById("right_choice").style.width="45%";
            ...
            //making it bigger and adjusting the size of the circle container similtaneously so all fits.
                ///STILL NEED TO MOVE IT UP AT THE SAME TIME SO DOESN'T JUST GROW AT THE BOTTOM...
            document.getElementById("left_choice").style.height="65%";document.getElementById("left_choice").style.width="45%";
...
function first_thing(){
        ...
        //$("#containing_container").css('height',$(window).height()/2);
        //add functions that get called by key presses or button presses
