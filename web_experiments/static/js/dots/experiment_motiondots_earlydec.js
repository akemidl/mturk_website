...
var num_dots=1500; //number of dots on screen at any time...
...
var total_instruction_screens=12; //12
var num_summaryscreen=1;
var num_practicetrials=5;
var num_trials_eachblock=40; //this is what we want to have in each of the blocks. Will make the same variables and then just shuffle them (shuffle speed and colour separately)
var num_slidertrainingtrials=80;
var num_blocktrials=80; //160 category boundary block trials...
var num_probetrials=40; //probe trials- slider but without feedback...
...
function MakeColourEachDot(){
  ...
  if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
    {for (i = 0; i <num_blocktrials; i++) {
      var colour_tmp=(0.5+rb_array_catboundblocks_80[i]*0.5*colour_difficulty/1000); //transforming the input from rb_array
      ...
      speed_eachdot.push(speed_tmp);
          ...
          {for (i = 0; i <num_trials_eachblock; i++) {
            var colour_tmp=(0.5+rb_array_40[i]*0.5*colour_difficulty/1000); //transforming the input from rb_array
...
function Positions_outcomebar(){
  if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)      {num_trials_thisblock=num_blocktrials;}  else {num_trials_thisblock=num_trials_eachblock;};    for (i=0;i<num_trials_thisblock;i++){
    ...
    function RunInstruct(event){
        ...
        //// Welcome Screen
        	if(currentnumber==1){
        		document.getElementById('instruct').innerHTML='<p>Welcome to the experiment!</p><p>On each trial in this task you will be asked about the average colour of moving coloured dots. The moving coloured dots will be shown for 0.5 seconds on each trial, before being covered by a grey circle. The colours of the dots and their speed will vary independently for each trial.</p><p><b>Because this task depends on colours, please turn off FLUX or any programs that change the colour of your screen before continuing.</b></p><p>Press continue</p>'
        ...
        //// Display first trial screen
          ...
          $('#container').show()
            ...
            document.getElementById('instruct').innerHTML = '<p>The boundary between the ‘more-blue’ and ‘more-red’ overall dot colour trials is fixed for the whole block, and will not change over the course of the block. However, the boundary may or may not change between blocks of blue-red trials.</p><p>You will receive a monetary bonus at the end of the experiment partly based on how many correct responses you make during these blocks.The reward bar at the bottom will increment when you are correct so you can keep track of how you are going</p><p>Press continue for an example of a ‘more-red’/’more-blue’ choice trial</p>'
            //document.getElementById('mag_right').style.border = '.2em solid black';
            $('#first_div').show();
            $('#second_div').show();
            $('#still_motion_dots').show();
            ...
            document.getElementById('instruct').innerHTML='<p>In summary:</p><p>There will be 1 block of the training trials where you will enter your choice on the blue-red scale using the slider, and receive feedback on the true overall blue-red dot colour on the blue-red scale.</p><p>There will be 2 blocks of the ‘more-blue’ or ‘more-red’ choice, where you have to decide whether the overall dot colour was ‘more-blue’ or ‘more-red’ than the arbitrary boundary. You can learn where the boundary is for that block from the feedback on each trial. Remember that the boundary is fixed for the entire block, but may potentially vary between blocks.</p><p>Between each of the ‘more-blue’ or ‘more-red’ choice blocks, there will be 2 blocks where you need to enter the precise overall dot colour on the blue-red scale using the slider.</p><p>After each block of trials there will be a break screen. You can take a break for as long as you like each time, and click the continue button with your mouse when you are ready to start again.</p><p>You will be rewarded with a monetary bonus based on your performance in all blocks other than the training block.</p><p>Press ‘continue’ when you are ready to start, or press ‘previous’ to read any of the instructions again if you are unsure.</p>'
            $('#first_div').show();
            $('#second_div').show();
            ...
            trials_counter = num_practicetrials;
  ...
  function RunAfterPracticeInstruct(event){
    ...
    if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
      ...
      {trials_counter= num_slidertrainingtrials}
        ...
        {console.log('trials_counter= total_instruction_screens'); trials_counter= total_instruction_screens}
...
$("#outcomebar").css("background-color","black")
  ...
  var outcomeslider=function(response){
      ...
      T4 =setTimeout(function() {
        ...
        tnum=currenttrial_t
        if (eventt.indexOf('block1trial')==0 || eventt.indexOf('block2trial')==0 || eventt.indexOf('block3trial')==0 || eventt.indexOf('block4trial')==0)//block 1 or block 2 trials
          {element.innerHTML='Completed: '+tnum+'/'+num_blocktrials}
          else if (eventt.indexOf('practicetrial_training')==0 || eventt.indexOf('practicetrial_catboundary')==0 || eventt.indexOf('practicetrial_probe')==0) {
            element.innerHTML='Completed: '+tnum+'/'+num_practicetrials}
        else
