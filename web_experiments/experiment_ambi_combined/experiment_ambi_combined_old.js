...
//// First Trial 
		...
		document.getElementById('instruct').innerHTML='<p>On each BLUE trial, you will choose between the the box on the left and the box on the right. In order to make your choice, you will need to consider two things. The first thing to consider is the number above each box, which indicates the number of points you might earn. If you were just considering the number of points, you would want to chose the box with the larger number.</p>'
		...
		document.getElementById('instruct').innerHTML='<p>The second thing to consider when making your choice is how likely it will be that you will receive the points above the box. This is determined by the number of ‘O’s in the box. Once you have made your choice, a token (an ‘X’ or ‘O’) is randomly drawn from the box you chose. If an ‘O’ is drawn, you may go on to receive those points. If an ‘X’ is drawn, you would receive 0 points.</p>'

		
	}else if(currentnumber==4){
		document.getElementById('instruct').innerHTML='<p>On some BLUE trials you may have to trade-off between the probability that an ‘O’ will be drawn, and how many points you might earn. Here for example, the box on the left has a lot less ‘O’s, but the number of points is higher. Conversely, you are more likely to get an ‘O’ if you choose the box on the right, but this is linked to less points. How you make this trade-off is up to you. </p>'
		...
		document.getElementById('instruct').innerHTML='<p>After you make your choice, a token will be drawn from the box. In this case, an ‘O’ was drawn associated with 11 points. This outcome will be added to the first slot of your temporary tally board in the top-middle of the screen. It is not immediately added to your total score.</p>'
		display_urns_and_mags(currenttrial_p,practice,0,0)
		outcome = '11'
...
//// tally explanation   //// 
		...
		hide_urns_and_mags()
		document.getElementById('instruct').innerHTML = '<p>Importantly, the boxes change independently on every trial, so you can’t use information about whether you got an ‘O’ or not to guide your behavior on the next trial.</p><p>Press continue and we will walk through the remaining 4 trials in this example block</p>'
		document.getElementById('mag_right').style.borderWidth = '5px';
		allow_instruct_to_press = false;
		$('#button_continue').show();
		$('#outcome').hide();
...
//// 2nd trial
		...
		currenttrial_p=1
		currenttrial_within_5=1	
		document.getElementById('instruct').innerHTML = 'Here the outcome was again an ‘O’, so the 2nd slot in the temporary tally board changes to show 75 points.'
		outcome='75'
		display_urns_and_mags(currenttrial_p,practice,0,0) // 
...
//// Third Trial  (X) 
		...
		document.getElementById('instruct').innerHTML = 'This third example is another type of trial you will encounter. Some of the tokens in one of the boxes are covered up by ‘=’ signs.  So you do not know whether there is an ‘X’ or an ‘O’ underneath each one. There are still 50 tokens in the box with a predefined proportion of ‘X’s and ‘O’s. This unknown proportion determines the likelihood that an ‘O’ will be drawn.'
		document.getElementById('mag_left').style.borderWidth = '1px';
		document.getElementById('mag_right').style.borderWidth = '1px';	
		document.getElementById('outcome').innerHTML=''		
		$('#outcome').hide()	
		...
		document.getElementById('instruct').innerHTML = 'The 20 uncovered tokens have been picked at random, so they give you some information about what is in the box. If the computer did it again, it might pick a different number of ‘O’s and ‘X’s even though you’re drawing from the same box with the same predefined underlying number of ‘X’s and ‘O’s. So, you have to infer the actual proportion from the sample you see.'


	}else if(currentnumber==14){		
		document.getElementById('instruct').innerHTML = '<p>The more tokens you can see, the more information you have and the more likely it will be that the ratio of ‘O’s to ‘X’s you can see reflects the actual proportion of ‘X’s to ‘O’s in the box.</p>'
		currenttrial_p=2
		currenttrial_within_5=2
		display_urns_and_mags(currenttrial_p,practice,1,0)
		...
		document.getElementById('instruct').innerHTML = '<p> Okay, so let’s assume you chose the one on the right. Press the rightward arrow key.</p>'
		currenttrial_p=2
		currenttrial_within_5=2
...
//// Fourth Trial  (0)
		...
		document.getElementById('instruct').innerHTML = '<p>Here, there is also missing information in one box (this time, the box on the left). This time, a lot of information is missing -- you can only see four tokens.</p><p>One minor point is that the ‘X’s are shuffled to the left in each row to make it easier for you to see how many are in each bin. But this happens after the ‘=’ signs are put down. So in a row that is partly hidden like the bottom row here, the hidden tokens can be either ‘X’s OR ‘O’s.</p><p>Go ahead and make your choice.</p>'
		document.getElementById('mag_left').style.borderWidth = '1px';	
		document.getElementById('mag_right').style.borderWidth = '1px';			
		practice=true
		$('#outcome').hide()
...
//// 5 Trial Summary 
		...
		document.getElementById('instruct').innerHTML = '<p>The computer will randomly select one of these outcomes. If you won some points, these will be added to your scoreboard. </p><p> For example, here the computer chose the outcome associated with 75 points, which have been added to your total score in the top right.</p>'
...
//// Do some negative examples //// 
		...
		document.getElementById('instruct').innerHTML='<p>Now we will walk through an example block of RED trials. On these trials, you will be trying to avoid losing points that you already have.</p>'
		$('#button_continue').show();
		reset_tally()
		hide_tally()
		$('#summary_text').hide();
...
///// 6th trial	
		...
		practice=true
		change_background_color('loss')
		document.getElementById('instruct').innerHTML = '<p> On this RED trial, notice that the numbers above the boxes are negative. This indicates points you might lose from your current total. Here, if an ‘O’ is drawn, the negative points will be entered in your temporary tally board. If that number is chosen at the end of the 5 trial block, those points will be subtracted from your total.</p><p>On this trial, the box on the right is more likely to result in a loss, because there are more ‘O’s, but the potential loss is smaller. </p><p>You can choose either side here.</p>'
		display_urns_and_mags(currenttrial_p,practice,1,0)
		...
		outcome = parseInt(mag[currenttrial_p].replace("'", "").replace("'",""))
		str1 = 	'<p>An ‘O’ was drawn from the box you picked. You can see that '
		str2 = outcome
		str1 = str1.concat(str2)
		document.getElementById('instruct').innerHTML = str1.concat(' points have been entered into the first slot in the temporary tally board. </p><p> Press Continue for another trial.</p>')
		document.getElementById('outcome').innerHTML='O'
		increment_tally(outcome)
		$('#outcome').show()
...
///// 7th trial	
		...
		document.getElementById('instruct').innerHTML = '<p>On this trial, the box on the left has a less likely loss, but it is larger. Conversely, the box on the right has a more likely loss, but it is smaller.</p><p>Here, you can again choose either side.</p>'
		display_urns_and_mags(currenttrial_p,practice,0,0)
		...
		outcome = parseInt(mag[currenttrial_p].replace("'", "").replace("'",""))
		document.getElementById('instruct').innerHTML = '<p>An ‘X’ was drawn from the box you picked, so a loss of 0 points (i.e. no loss) has been entered into the second slot in the temporary tally board.</p><p> Press Continue.</p>'
		document.getElementById('outcome').innerHTML='X'
		increment_tally(0)
		$('#outcome').show()
...
///// 8th trial	
		...
		document.getElementById('instruct').innerHTML = 'Here, the box on the right has a less likely loss and it is smaller. You should probably choose that side.'
		display_urns_and_mags(currenttrial_p,practice,1,0)
		...
		outcome = parseInt(mag[currenttrial_p].replace("'", "").replace("'",""))
		document.getElementById('instruct').innerHTML = '<p>Here, another ‘X’  was drawn, so a loss of 0 has been entered into your tally board.</p>'
		document.getElementById('outcome').innerHTML='X'
		increment_tally(0)
		$('#outcome').show()
...
//// 9th Trial 
		...
		currenttrial_p=8
		currenttrial_within_5=3	
		document.getElementById('instruct').innerHTML = '<p>Here is another example where some tokens are covered up. Based on what you can see, you have to estimate how likely it is an ‘O’ will be drawn from the box on the right.</p><p>You can choose either side here.</p>'
		document.getElementById('mag_left').style.borderWidth = '1px';	
		document.getElementById('mag_right').style.borderWidth = '1px';			
		practice=true
		$('#outcome').hide()
		...
		outcome = parseInt(mag[currenttrial_p].replace("'", "").replace("'",""))
		document.getElementById('instruct').innerHTML = '<p>An ‘O’ was drawn from the box you picked, so a loss has been entered into the fourth slot in the temporary tally board.</p><p> Press Continue to get the fifth trial from this example block.</p>'
		document.getElementById('outcome').innerHTML='O'
		increment_tally(outcome)
		$('#outcome').show()
		allow_instruct_to_press = false; 
...
//// 5 Trial Summary (loss) 
		...
		document.getElementById('instruct').innerHTML = 'Just like before, after 5 RED trials you will see a screen with just the temporary tally board, showing your outcomes for the previous 5 trials.'
		/// Display text .. 
			var element = document.getElementById("summary_text");
			element.innerHTML = 'The computer will now choose one of your previous 5 outcomes.'
			$('#summary_text').show();
		...
		str2 = outcome
		str1 = str1.concat(str2)
		document.getElementById('instruct').innerHTML = str1.concat(' points, which have been subtracted from your scoreboard in the top right.</p>')
...
//// Prepare them for 5 more practice trials //// 
		...
		document.getElementById('instruct').innerHTML='<p>Now, we would like you to complete 10 more practice trials on your own. </p><p> Remember to respond as soon as you’ve made your decision.</p><p>If anything is still unclear, press the ‘Go Back’ button to go back through each screen or refresh your browser to go to the beginning of the instructions.</p><p>Press continue and get started!</p>'
		reset_tally()		
		hide_tally()
		$('#summary_text').hide();
		$('#outcome5').hide()
...
//// Intsruction summary  //// 
		...
		$('#button_continue').show();
		change_background_color('neutral')
		document.getElementById('instruct').innerHTML='<p>On some trials there will be missing information for one box, where some of the tokens are covered up with ‘=’ signs, and hence you can only see some of the tokens in that box, randomly chosen from the box as a whole. You must do the best you can with the information that you have.</p><p>There will be 300 trials. You will get 5 breaks, one after each 50 trials.</p><p>STOP! Are you ready to start? This is a timed experiment, so once you click continue, you will not be able to go back.Press ‘Continue’ to get started on the task. Good luck!</p>'
