...
picture{
	text{caption = " "; font_size = 40; font_color=0,0,0,127;}result_text;
	x = 0; y = 0;
}result_picture;
...
shocklookup.open("S01"+"_pain_scale.dat",bool(1)); 
loop until
...
int ntrials=30; #number of trials in a block
...
int timeout_choice=300000; #time during which subjects may choose which to bet on, in milliseconds
...
double p_left=input_file_piloting_demo [t][4];
double p_right=input_file_piloting_demo [t][3]; #proportion of the balls that are blue
double pr=input_file_piloting_demo [t][5]; #proportion of the balls that are revealed (ie not 'covered' by "!") initially on the right urn
double pr_left=input_file_piloting_demo [t][6]; #proportion of the balls that are revealed (ie not 'covered' by "!") initially on the left

#These times are really long so in demo version need to go between screens by pressing enter- will only time out after really long time.
double UrnsPresented_duration=30000000.0; #Screen '2'
double ChoiceTime_duration=30000000.0;
double ChoiceDisplayed_duration=30000000.0;
double OutcomeDisplayed_duration=30000000.0;
double ITI_duration=300000000.0; #Screen '1'
double OutcomeHistoryDisplayed_duration=300000000.0; #Only every 10 trials...
double ShockOutcomeDisplayed_duration=30000000000.0;
double ExtraITI_duration=3000000000000.0;

int outcome_box_x=-145;
int outcome_box_border_x=-145;
...
#=====================================================================================================
	...
	p_right=input_file_piloting_demo [t][3]; #need to redefine as participants were allowed to change these on the last trial.
	p_left=input_file_piloting_demo [t][4];
	pr=input_file_piloting_demo [t][5];
	pr_left=input_file_piloting_demo [t][6];
	reveal=false;
	outcome_icon=false;
	
	#Setting magnitude captions appropriately from values in array
	string magnitude_left_string=string(input_file_piloting_demo[t][1]);
	string magnitude_right_string=string(input_file_piloting_demo[t][2]);
...
#set colours of balls for this trial from input array:
	...
	begin
		...
		colours[c]=colours_toinput_demo[t][c];
		c=c+1;
	end;
...
#First trial or on break times we need a brief ITI a la Mike just so the first thing they see is not the urns. Will they also need an instruction screen?
	...
	int r=response_manager.total_response_count(); #defining r. Now we want to use this so it stops when someone presses enter.
	flag=false;
	loop int start_time1_1=clock.time();
	until (flag==true || (clock.time()-start_time1_1)>int(UrnsPresented_duration))
	begin 
		urn.present();
		if response_manager.total_response_count()>r then
			if response_manager.last_response()==3 then flag=true; #They pressed enter so we want it to go to next screen...
			end;
		end;
	end;
...
#PARTICIPANTS CAN CHOOSE WHETHER TO BET ON CROSSES IN EITHER URN===========================================================================
	...
	r=response_manager.total_response_count(); #defining r. 
	flag=false;
	...
	until (flag==true ||(clock.time()-start_time2)>int(ChoiceTime_duration))
	begin 
		urn.present();
		if response_manager.total_response_count()>r then
			if response_manager.last_response()==1 then outcome_box_x=-145; outcome_box_border_x=-145; mr=mr2; br=br_left; button_press=string(clock.time()-trial_starttime); choice_time=(clock.time()-start_time2); #int rr=response_manager.total_response_count(); #THEY SELECTED LEFT URN 

			elseif response_manager.last_response()==2 then outcome_box_x=145; outcome_box_border_x=145; mr=mr1; br=br_right; button_press=string(clock.time()-trial_starttime); choice_time=(clock.time()-start_time2); #int rr=response_manager.total_response_count(); #THEY SELECTED RIGHT URN 

			elseif response_manager.last_response()==3 then flag=true;
			end;
		end;
...
#Presenting the urns with the square indicating the participants/computers choice.
	...
	until (flag==true || (clock.time()-start_time3)>int(ChoiceDisplayed_duration))
	begin
	urn.present();
		if response_manager.total_response_count()>r then
			if response_manager.last_response()==3 then flag=true;
			end;
		end;
	end;
...
############################DETERMINING OUTCOME########################################################################################	
	...
	double mag_left=input_file_piloting_demo[t][1]; #magnitudes of red and blue as doubles (from array)
	double mag_right=input_file_piloting_demo[t][2];
...
####On 6th trial etc need to reset for this new block of ten...##########################
...
loop int start_time4=clock.time();
	until (flag==true || (clock.time()-start_time4)>int(OutcomeDisplayed_duration))
	begin
		urn.present();
		if response_manager.total_response_count()>r then
			if response_manager.last_response()==3 then flag=true;
...
##########GIVE THEM THE OUTCOME THEY ACCUMULATE EVERY 5 TRIALS###############################
...
#screen here where it has the result bins but none of them are highlighted yet and the info about the magnitude shock they will get is not displayed...
			...
			loop int start_time5=clock.time();
				until (flag==true || (clock.time()-start_time5)>int(OutcomeHistoryDisplayed_duration))
				begin
...
########Screen 7#####################################################################################################
			...
			loop int start_time6=clock.time();
				until (flag==true ||(clock.time()-start_time6)>int(ShockOutcomeDisplayed_duration))
				begin
				result_picture.present();
				if response_manager.total_response_count()>r then
					if response_manager.last_response()==3 then flag=true;
...
#############################SCREEN 8: WIll need an extra ITI after every 10 trials (the long ITI...)
	...
	until (flag==true || (clock.time()-start_time7)>int(ExtraITI_duration))
	begin
...
string intertrialinterval="blah_de_blah";
if mod(t,5)>0 #i.e. if not right after they have been shocked and had the long ITI...
then
...
loop int start_time1=clock.time();
	until (flag==true ||(clock.time()-start_time1)>int(ITI_duration))
	begin
	default.present();
		if response_manager.total_response_count()>r then
			if response_manager.last_response()==3 then flag=true;
